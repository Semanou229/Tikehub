...
<div class="p-3 sm:p-4 lg:p-6">
    ...
    <div class="bg-white rounded-lg shadow-md mb-4 sm:mb-6">
        <div class="flex flex-wrap border-b border-gray-200">
            <a href="{{ route('buyer.tickets', ['tab' => 'tickets'] + request()->except('tab', 'page')) }}" 
               class="flex-1 sm:flex-none px-3 sm:px-6 py-3 sm:py-4 text-center text-xs sm:text-sm font-medium border-b-2 transition {{ $activeTab === 'tickets' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} min-h-[44px] flex items-center justify-center">
                <i class="fas fa-ticket-alt mr-1.5 sm:mr-2 text-xs sm:text-sm"></i>
                <span>Billets</span>
                @if($ticketsCount > 0)
                    <span class="ml-1.5 sm:ml-2 bg-indigo-100 text-indigo-600 text-xs px-2 py-0.5 rounded-full">{{ $ticketsCount }}</span>
                @endif
            </a>
            <a href="{{ route('buyer.tickets', ['tab' => 'votes'] + request()->except('tab', 'page')) }}" 
               class="flex-1 sm:flex-none px-3 sm:px-6 py-3 sm:py-4 text-center text-xs sm:text-sm font-medium border-b-2 transition {{ $activeTab === 'votes' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} min-h-[44px] flex items-center justify-center">
                <i class="fas fa-vote-yea mr-1.5 sm:mr-2 text-xs sm:text-sm"></i>
                <span>Votes</span>
                @if($votesCount > 0)
                    <span class="ml-1.5 sm:ml-2 bg-indigo-100 text-indigo-600 text-xs px-2 py-0.5 rounded-full">{{ $votesCount }}</span>
                @endif
            </a>
            <a href="{{ route('buyer.tickets', ['tab' => 'donations'] + request()->except('tab', 'page')) }}" 
               class="flex-1 sm:flex-none px-3 sm:px-6 py-3 sm:py-4 text-center text-xs sm:text-sm font-medium border-b-2 transition {{ $activeTab === 'donations' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} min-h-[44px] flex items-center justify-center">
                <i class="fas fa-heart mr-1.5 sm:mr-2 text-xs sm:text-sm"></i>
                <span>Dons</span>
                @if($donationsCount > 0)
                    <span class="ml-1.5 sm:ml-2 bg-indigo-100 text-indigo-600 text-xs px-2 py-0.5 rounded-full">{{ $donationsCount }}</span>
                @endif
            </a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-3 sm:p-4 mb-4 sm:mb-6">
        <form method="GET" action="{{ route('buyer.tickets') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            ...
            <div class="sm:col-span-2 lg:col-span-1">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Rechercher..." class="w-full px-3 sm:px-4 py-2 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 min-h-[44px]">
            </div>
            ...
            <div class="flex gap-2 items-end">
                ...
                @if(request('search') || request('status'))
                    <a href="{{ route('buyer.tickets', ['tab' => $activeTab]) }}" class="bg-gray-200 text-gray-700 px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg hover:bg-gray-300 active:bg-gray-400 transition min-w-[44px] min-h-[44px] flex items-center justify-center">
    ...
    @elseif($activeTab === 'votes')
                    ...
                    <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition">
                        ...
                        <div class="p-4 sm:p-6">
                            ...
                            <div class="flex items-center justify-end pt-3 sm:pt-4 border-t border-gray-200">
                                <a href="{{ route('contests.show', $vote->contest) }}" class="text-indigo-600 hover:text-indigo-800 active:text-indigo-700 px-3 sm:px-4 py-2 rounded-lg hover:bg-indigo-50 text-xs sm:text-sm font-medium min-h-[36px] flex items-center justify-center">
        ...
        @else
            <div class="bg-white rounded-lg shadow-md p-6 sm:p-8 lg:p-12 text-center">
                ...
                <a href="{{ route('contests.index') }}" class="inline-block bg-indigo-600 text-white px-4 sm:px-5 lg:px-6 py-2.5 sm:py-3 rounded-lg hover:bg-indigo-700 active:bg-indigo-800 transition font-medium text-xs sm:text-sm lg:text-base min-h-[40px] sm:min-h-[44px] flex items-center justify-center shadow-md hover:shadow-lg">
    ...
    @elseif($activeTab === 'donations')
                ...
                @foreach($donations as $donation)
                    <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition">
                        ...
                        <div class="p-4 sm:p-6">
                            ...
                            <div class="flex items-center justify-end pt-3 sm:pt-4 border-t border-gray-200">
                                <a href="{{ route('fundraisings.show', $donation->fundraising) }}" class="text-indigo-600 hover:text-indigo-800 active:text-indigo-700 px-3 sm:px-4 py-2 rounded-lg hover:bg-indigo-50 text-xs sm:text-sm font-medium min-h-[36px] flex items-center justify-center">
        ...
        @else
            <div class="bg-white rounded-lg shadow-md p-6 sm:p-8 lg:p-12 text-center">
                ...
                <a href="{{ route('fundraisings.index') }}" class="inline-block bg-indigo-600 text-white px-4 sm:px-5 lg:px-6 py-2.5 sm:py-3 rounded-lg hover:bg-indigo-700 active:bg-indigo-800 transition font-medium text-xs sm:text-sm lg:text-base min-h-[40px] sm:min-h-[44px] flex items-center justify-center shadow-md hover:shadow-lg">
