...
@section('title', 'Mes Événements Virtuels')
...
<div class="p-3 sm:p-4 lg:p-6">
                    ...
                    <div class="p-4 sm:p-6">
                        <div class="flex items-start justify-between mb-2 sm:mb-3">
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs sm:text-sm text-gray-600 flex items-center truncate">
                                    ...
                                    <span class="truncate">{{ ucfirst(str_replace('_', ' ', $ticket->event->platform_type ?? 'Visioconférence')) }}</span>
                                </p>
                            </div>
                        ...
                        <div class="space-y-1.5 sm:space-y-2 mb-3 sm:mb-4 text-xs sm:text-sm text-gray-600">
                            ...
                            <div class="flex items-center">
                                ...
                                <span class="truncate">Durée: {{ $ticket->event->start_date->diffForHumans($ticket->event->end_date, true) }}</span>
                            </div>
                            @if($ticket->event->virtual_access_instructions)
                                <div class="mt-2 p-2 sm:p-3 bg-blue-50 rounded text-xs text-gray-700">
                                    <i class="fas fa-info-circle text-blue-600 mr-1"></i>
                                    {{ \Illuminate\Support\Str::limit($ticket->event->virtual_access_instructions, 80) }}
                                </div>
                            @endif
                            @if($ticket->event->meeting_password)
                                <div class="flex items-center text-xs">
                                    ...
                                    <span class="truncate">Mot de passe: <strong>{{ $ticket->event->meeting_password }}</strong></span>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="space-y-2 pt-3 sm:pt-4 border-t border-gray-200">
                            @if($ticket->virtual_access_token)
                                @if($ticket->event->start_date >= now())
                                    <a href="{{ $ticket->getVirtualAccessUrl() }}" target="_blank" class="block w-full bg-blue-600 text-white text-center px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg hover:bg-blue-700 active:bg-blue-800 transition font-medium text-xs sm:text-sm mb-2 min-h-[44px] flex items-center justify-center shadow-sm hover:shadow-md">
                            ...
                            <div class="flex items-center justify-between gap-2">
                                <a href="{{ route('tickets.show', $ticket) }}" class="text-indigo-600 hover:text-indigo-800 active:text-indigo-700 text-xs sm:text-sm font-medium min-h-[36px] flex items-center justify-center px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg hover:bg-indigo-50 transition">
                                ...
                                <a href="{{ route('tickets.download', $ticket) }}" class="text-green-600 hover:text-green-800 active:text-green-700 text-xs sm:text-sm font-medium min-h-[36px] flex items-center justify-center px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg hover:bg-green-50 transition">
    ...
    @else
        <div class="bg-white rounded-lg shadow-md p-6 sm:p-8 lg:p-12 text-center">
            ...
            <a href="{{ route('home') }}" class="inline-block bg-indigo-600 text-white px-4 sm:px-5 lg:px-6 py-2.5 sm:py-3 rounded-lg hover:bg-indigo-700 active:bg-indigo-800 transition font-medium text-xs sm:text-sm lg:text-base min-h-[40px] sm:min-h-[44px] flex items-center justify-center shadow-md hover:shadow-lg mx-auto">
