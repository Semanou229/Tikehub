...
<div class="p-3 sm:p-4 lg:p-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0 mb-4 sm:mb-6">
        ...
        <a href="{{ route('organizer.crm.sponsors.index') }}" class="text-indigo-600 hover:text-indigo-800 text-sm sm:text-base min-h-[44px] flex items-center">
    ...
    <div class="bg-white rounded-lg shadow-md p-3 sm:p-4 lg:p-6">
        <form action="{{ route('organizer.crm.sponsors.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="company" value="{{ old('company') }}" class="w-full px-3 sm:px-4 py-2 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 min-h-[44px]">
                </div>

                <div>
                    <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Email</label>
                    <input type="email" name="email" value="{{ old('email') }}" class="w-full px-3 sm:px-4 py-2 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 min-h-[44px]">
                ...
                <div>
                    ...
                    <input type="text" name="phone" value="{{ old('phone') }}" class="w-full px-3 sm:px-4 py-2 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 min-h-[44px]">
                </div>
                ...
                <div>
                    ...
                    <select name="sponsor_type" required class="w-full px-3 sm:px-4 py-2 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 min-h-[44px]">
                        <option value="gold" {{ old('sponsor_type') == 'gold' ? 'selected' : '' }}>Gold</option>
                        <option value="silver" {{ old('sponsor_type') == 'silver' ? 'selected' : '' }}>Silver</option>
                        <option value="bronze" {{ old('sponsor_type') == 'bronze' ? 'selected' : '' }}>Bronze</option>
                        <option value="partner" {{ old('sponsor_type') == 'partner' ? 'selected' : '' }}>Partenaire</option>
                        <option value="media" {{ old('sponsor_type') == 'media' ? 'selected' : '' }}>Média</option>
                        <option value="other" {{ old('sponsor_type') == 'other' ? 'selected' : '' }}>Autre</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="status" required class="w-full px-3 sm:px-4 py-2 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 min-h-[44px]">
                        ...
                        <option value="negotiating" {{ old('status') == 'negotiating' ? 'selected' : '' }}>En négociation</option>
                ...
                <div>
                    ...
                    <input type="number" name="contribution_amount" value="{{ old('contribution_amount', 0) }}" step="0.01" min="0" required class="w-full px-3 sm:px-4 py-2 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 min-h-[44px]">
                    @error('contribution_amount')<p class="text-red-500 text-xs sm:text-sm mt-1">{{ $message }}</p>@enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="contract_start_date" value="{{ old('contract_start_date') }}" class="w-full px-3 sm:px-4 py-2 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 min-h-[44px]">
                </div>

                <div>
                    <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Date fin contrat</label>
                    <input type="date" name="contract_end_date" value="{{ old('contract_end_date') }}" class="w-full px-3 sm:px-4 py-2 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 min-h-[44px]">
                </div>

                <div class="md:col-span-2">
                    <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Avantages accordés</label>
                    <textarea name="benefits" rows="4" class="w-full px-3 sm:px-4 py-2 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">{{ old('benefits') }}</textarea>
                </div>

                <div class="md:col-span-2">
                    <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Notes</label>
                    <textarea name="notes" rows="3" class="w-full px-3 sm:px-4 py-2 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">{{ old('notes') }}</textarea>
                </div>
            </div>
            ...
            <div class="mt-6 sm:mt-8 flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 sm:gap-4">
                ...
                <a href="{{ route('organizer.crm.sponsors.index') }}" class="text-gray-600 hover:text-gray-800 active:text-gray-900 text-center sm:text-left py-2.5 sm:py-0 text-xs sm:text-sm min-h-[40px] sm:min-h-[44px] flex items-center justify-center sm:justify-start">
                    Annuler
                </a>
            </div>
        </form>
