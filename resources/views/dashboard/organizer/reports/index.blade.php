...
<div class="p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="text-3xl font-bold text-indigo-600">{{ $stats['total_events'] }}</div>
        </div>
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="text-sm text-gray-600 mb-1">Billets vendus</div>
            <div class="text-3xl font-bold text-green-600">{{ $stats['total_tickets_sold'] }}</div>
        </div>
        <div class="bg-white rounded-lg shadow-md p-6">
                ...
                @foreach($events as $event)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <a href="{{ route('organizer.reports.event', $event) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">
                                <i class="fas fa-eye mr-1"></i>Voir
                            </a>
                            <a href="{{ route('organizer.reports.export', [$event, 'csv']) }}" class="text-green-600 hover:text-green-900 mr-3">
                                <i class="fas fa-download mr-1"></i>CSV
                            </a>
                            <a href="{{ route('organizer.reports.export', [$event, 'excel']) }}" class="text-blue-600 hover:text-blue-900">
