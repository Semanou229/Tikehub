...
<div class="p-6">
    <div class="mb-6">
        ...
        <h1 class="text-3xl font-bold text-gray-800">Modifier le code promo</h1>
    </div>

    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('organizer.promo-codes.update', $promoCode) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="event_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        ...
                        @foreach($events as $event)
                            <option value="{{ $event->id }}" {{ old('event_id', $promoCode->event_id) == $event->id ? 'selected' : '' }}>
                                {{ $event->title }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3" 
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                              placeholder="Description du code promo...">{{ old('description', $promoCode->description) }}</textarea>
                ...
                <div>
                    ...
                    <select name="discount_type" id="discount_type" required 
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        <option value="percentage" {{ old('discount_type', $promoCode->discount_type) == 'percentage' ? 'selected' : '' }}>Pourcentage (%)</option>
                        <option value="fixed" {{ old('discount_type', $promoCode->discount_type) == 'fixed' ? 'selected' : '' }}>Montant fixe (XOF)</option>
                    </select>
                ...
                <div>
                    ...
                    <div class="flex items-center">
                        <input type="number" name="discount_value" value="{{ old('discount_value', $promoCode->discount_value) }}" required step="0.01" min="0.01"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                               placeholder="10" id="discount_value">
                        <span class="ml-2 text-gray-600" id="discount_unit">{{ $promoCode->discount_type === 'percentage' ? '%' : 'XOF' }}</span>
                    </div>
                ...
                <div>
                    ...
                    <input type="number" name="minimum_amount" value="{{ old('minimum_amount', $promoCode->minimum_amount) }}" step="0.01" min="0"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                           placeholder="0">
                ...
                <div id="max_discount_container" style="display: {{ $promoCode->discount_type === 'percentage' ? 'block' : 'none' }}">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Réduction maximum (optionnel)</label>
                    <input type="number" name="maximum_discount" value="{{ old('maximum_discount', $promoCode->maximum_discount) }}" step="0.01" min="0"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                           placeholder="0">
                ...
                <div>
                    ...
                    <input type="datetime-local" name="start_date" value="{{ old('start_date', $promoCode->start_date ? $promoCode->start_date->format('Y-m-d\TH:i') : '') }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                ...
                <div>
                    ...
                    <input type="datetime-local" name="end_date" value="{{ old('end_date', $promoCode->end_date ? $promoCode->end_date->format('Y-m-d\TH:i') : '') }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
