...
<div class="p-6">
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('collaborator.events.show', $event) }}" class="text-indigo-600 hover:text-indigo-800">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="text-3xl font-bold text-green-600">{{ $stats['my_valid_scans'] }}</div>
        </div>
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="text-sm text-gray-600 mb-1">Tentatives invalides</div>
            <div class="text-3xl font-bold text-red-600">{{ $stats['my_invalid_scans'] }}</div>
        </div>
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="text-sm text-gray-600 mb-1">Total billets</div>
            <div class="text-3xl font-bold text-indigo-600">{{ $stats['total_tickets'] }}</div>
        </div>
    </div>
...
function handleScan(qrToken, code = null) {
    ...
    fetch('{{ route("collaborator.scans.scan", $event) }}', {
        method: 'POST',
        body: formData
    })
