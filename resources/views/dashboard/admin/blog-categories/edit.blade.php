...
@section('title', 'Modifier une Catégorie de Blog')

@section('content')
<div class="p-6">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Modifier une Catégorie de Blog</h1>
    </div>

    <form action="{{ route('admin.blog-categories.update', $blogCategory) }}" method="POST" class="bg-white rounded-lg shadow-md p-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            <div>
                ...
                <input type="text" name="name" id="name" required value="{{ old('name', $blogCategory->name) }}"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none">{{ old('description', $blogCategory->description) }}</textarea>
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label for="color" class="block text-sm font-semibold text-gray-700 mb-2">Couleur</label>
                    <input type="color" name="color" id="color" value="{{ old('color', $blogCategory->color) }}"
