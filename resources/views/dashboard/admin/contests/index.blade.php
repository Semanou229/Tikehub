...
<div class="p-6">
    ...
    <div class="bg-white rounded-lg shadow-md p-4 mb-6">
        <form method="GET" action="{{ route('admin.contests.index') }}" class="flex gap-4">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($contests as $contest)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4">
                                    <div class="font-semibold text-gray-900">{{ $contest->name }}</div>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-900">
                                    {{ $contest->organizer->name ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-900">
                                    {{ $contest->votes_count }}
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-900">
                                    {{ $contest->candidates_count }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {{ $contest->end_date->translatedFormat('d M Y') }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <a href="{{ route('admin.contests.show', $contest) }}" class="text-red-600 hover:text-red-900">
