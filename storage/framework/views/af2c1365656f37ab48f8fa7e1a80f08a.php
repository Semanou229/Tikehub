                    ...
                    <?php if($event->venue_name || $event->venue_address): ?>
                        <div class="space-y-2 text-gray-700">
                            <?php if($event->venue_name): ?>
                                <p class="font-semibold"><i class="fas fa-map-marker-alt text-purple-600 mr-2"></i><?php echo e($event->venue_name); ?></p>
                            <?php endif; ?>
                                ...
                                <?php if($contest->isActive()): ?>
                                    <?php if(auth()->guard()->check()): ?>
                                        <form action="<?php echo e(route('contests.vote', ['contest' => $contest, 'candidate' => $candidate])); ?>" method="POST" class="space-y-3">
                                            <?php echo csrf_field(); ?>
                                            <div class="space-y-2">
                                                <label for="quantity_<?php echo e($candidate->id); ?>" class="block text-sm font-medium text-gray-700">Nombre de votes:</label>
                                                <div class="flex items-center gap-2">
                                                    <button type="button" onclick="decrementVote(<?php echo e($candidate->id); ?>)" class="w-10 h-10 bg-gray-200 hover:bg-gray-300 rounded-lg flex items-center justify-center transition">
                                                        <i class="fas fa-minus text-gray-600"></i>
                                                    </button>
                                                    <input type="number" name="quantity" id="quantity_<?php echo e($candidate->id); ?>" min="1" max="100" value="1" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg text-center font-semibold" required>
                                                    <button type="button" onclick="incrementVote(<?php echo e($candidate->id); ?>)" class="w-10 h-10 bg-gray-200 hover:bg-gray-300 rounded-lg flex items-center justify-center transition">
                                            ...
                                            <div class="bg-purple-50 border-2 border-purple-200 rounded-lg p-3">
                                                <div class="flex justify-between items-center">
                                                    ...
                                                    <span class="text-xl font-bold text-purple-600" id="amount_<?php echo e($candidate->id); ?>"><?php echo e(number_format($contest->price_per_vote, 0, ',', ' ')); ?> XOF</span>
                                                </div>
                                                <p class="text-xs text-gray-500 mt-1 text-center">
                                                    <?php echo e(number_format($contest->price_per_vote, 0, ',', ' ')); ?> XOF × <span id="total_<?php echo e($candidate->id); ?>">1</span> vote(s)
                                                </p>
                                            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        <?php if($event->venue_latitude && $event->venue_longitude): ?>
            ...
            L.marker([lat, lng])
                .addTo(map)
                .bindPopup('<?php echo e($event->venue_name ?? $event->venue_city ?? "Lieu du concours"); ?>')
                .openPopup();
            ...
            fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(fullAddress)}&limit=1`)
                ...
                .then(data => {
                    if (data && data.length > 0) {
                        ...
                        L.marker([lat, lng])
                            .addTo(map)
                            .bindPopup('<?php echo e($event->venue_name ?? $event->venue_address ?? "Lieu du concours"); ?>')
                            .openPopup();
                    }
                })
